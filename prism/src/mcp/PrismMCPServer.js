...
export class PrismMCPServer {
    ...
    getToolDefinitions() {
        return [
            ...
            {
                name: 'search_repo',
                description: 'Search the indexed codebase for relevant code snippets using semantic search. Returns the most similar code chunks with their similarity scores.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        query: {
            ...
            {
                name: 'get_context',
                description: 'Get context for a specific file, including all chunks from that file with their line numbers and content.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        filePath: {
            ...
            {
                name: 'explain_usage',
                description: 'Get usage information for a symbol (function, class, variable) including its definition and all places where it is used.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        symbol: {
            ...
            {
                name: 'list_indexed_files',
                description: 'List all files that are currently indexed in the database, grouped by language.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        language: {
                            type: 'string',
                            description: 'Filter by language (e.g., "typescript", "python"). If not provided, shows all languages.',
                        },
                    },
                },
            },
            ...
            {
                name: 'get_chunk',
                description: 'Get a specific chunk by ID. Useful when you have a chunk ID from a previous search and need the full content.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        chunkId: {
    ...
    async searchRepo(args) {
        ...
        const limit = args.limit || this.maxResults;
    ...
    async getContext(args) {
        ...
        const formatted = fileChunks.map((chunk) => `Lines ${chunk.startLine}-${chunk.endLine}:\n${this.indentCode(chunk.content, 2)}`);
    ...
    async explainUsage(args) {
        ...
        matchingChunks.forEach((chunk, i) => {
            ...
            if (definition && chunk.id === definition.id)
                return;
    ...
    async getAllChunks() {
        ...
        if ('getAllChunks' in this.vectorDB) {
            return await this.vectorDB.getAllChunks();
        }
